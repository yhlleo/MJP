...
class Attention(nn.Module):
    ...
    def forward(self, x, get_attn=False):
        ...
        dot_qk = (q @ k.transpose(-2, -1)) * self.scale
        attn = dot_qk.softmax(dim=-1)
        attn = self.attn_drop(attn)

        head = (attn @ v).transpose(1, 2)
        x = head.reshape(B, N, C)
        ...
        if not get_attn:
            ...
            return x, {'attn': attn, 'dot_qk': dot_qk, 'qkv':qkv, 'head': head}
...
class Block(nn.Module):
    ...
    def forward(self, x, get_attn=False):
        if not get_attn:
            ...
            y, attn = self.attn(self.norm1(x), get_attn=True) 
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.use_idx_emb = use_idx_emb
        self.use_dlocr = use_dlocr
        if self.use_dlocr:
            self.dlocr = DenseLocRegress(embed_dim, mlp_type=dlocr_type)
        ...
        if self.use_unk:
            num_unk = 1 
            self.pos_embed = nn.Embedding(num_patches + self.num_tokens + num_unk, embed_dim)
            self.unk_embed_index = num_patches + self.num_tokens + num_unk - 1
        else:
            self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + self.num_tokens, embed_dim))

        if self.use_idx_emb:
            self.index_embed = nn.Parameter(torch.zeros(1, num_patches + self.num_tokens, embed_dim))
    ...
    def forward_features(self, x, unk_mask=None):
        ...
        unk_mask: torch.Tensor, [N, seq_len]
        """
        ...
        if self.use_dlocr:
            if unk_mask is not None:
                xy, predxy = self.dlocr(self.pos_embed, (1-unk_mask)[:,1:])
            else:
                xy, predxy = self.dlocr(self.pos_embed, torch.ones(1, x.size(1) - 1))
        ...
        for i, blk in enumerate(self.blocks):
            if i < len(self.blocks) - 1:
                ...
                x, attn = blk(x, get_attn=True)
        x = self.norm(x)
        return self.pre_logits(x[:, 0]), attn, xy, predxy

    def test(self, x, unk_mask=None):
        x = self.patch_embed(x)
        cls_token = self.cls_token.expand(x.shape[0], -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        x = torch.cat((cls_token, x), dim=1)
        if self.use_unk:
    ...
    def forward(self, x, unk_mask=None):
        x, _, xy, predxy = self.forward_features(x, unk_mask)
        x = self.head(x)
        outs = Munch(
            sup=x, 
            xy=xy,
