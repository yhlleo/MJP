...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        if self.ape:
            if self.use_unk:
                num_unk = 1 
                self.absolute_pos_embed = nn.Embedding(num_patches + num_unk, embed_dim)
                self.unk_embed_index = num_patches + num_unk - 1
            else:
                self.absolute_pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim))
                trunc_normal_(self.absolute_pos_embed, std=.02)  

            if self.use_idx_emb:
                self.index_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim))
        ...
        if self.ape:
            if self.use_dlocr:
                if unk_mask is not None:
                    unk_mask = unk_mask[:,1:]
                    xy, predxy = self.dlocr(self.absolute_pos_embed, 1-unk_mask)
                else:
                    xy, predxy = self.dlocr(self.absolute_pos_embed, torch.ones(1, x.size(1)))

            if self.use_unk:
                seq_ord = torch.arange(x.size(1)).unsqueeze(0).to(x.device)     
                if unk_mask is not None: 
                    seq_ord = seq_ord * (1 - unk_mask) + unk_mask * self.unk_embed_index
                pos_embed = self.absolute_pos_embed(seq_ord)
            else:
    ...
    def forward(self, x, unk_mask=None):
        xs, xy, predxy = self.forward_features(x, unk_mask)
